...
class Vector:
    ...
    def __repr__(self):
        ...
        # repr(x)  ==>  x.__repr__()
        # f'Vector({", ".join(str(i) for i in self.coords)})'
        # ", ".join(str(i) for i in self.coords)
        return "Vector({0})".format(", ".join(str(i) for i in self.coords))

    def __str__(self):
        # human-readable representation
        # str(x)  ==> x.__str__()
        # print(x)  ==>  print(x.__str__())
        # f'({" ".join(str(i) for i in self.coords)})'
...
if __name__ == "__main__":
    ...
    # 2 body, more erratic
    # sim = GraphicalSimulation([
    #         # mass, position, initial_velocity=None
